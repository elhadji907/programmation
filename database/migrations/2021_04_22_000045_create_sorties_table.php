...
class CreateSortiesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create($this->tableName, function (Blueprint $table) {
            ...
            $table->index(["employees_id"], 'fk_sortie_employees1_idx');

            $table->index(["vehicules_id"], 'fk_sorties_vehicules1_idx');
            $table->softDeletes();
            $table->nullableTimestamps();


            $table->foreign('employees_id', 'fk_sortie_employees1_idx')
                ->references('id')->on('employees')
                ->onDelete('no action')
                ->onUpdate('no action');

            $table->foreign('vehicules_id', 'fk_sorties_vehicules1_idx')
