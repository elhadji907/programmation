...
$factory->define(App\Fad::class, function (Faker $faker) {
    return [
        'uuid' => $faker->uuid,
        'numero' => $faker->word,
...
$autoIncremente_fad = autoIncremente_fad();

$factory->define(App\Fad::class, function (Faker\Generator $faker) use ($autoIncremente_fad) {
    $autoIncremente_fad->next();

    $types_courrier_id=App\TypesCourrier::where('name','FADS')->first()->id;
    $annee = date('y');
    $numero_courrier = date('His');
    
    return [
        'numero' => 'CD'.$autoIncremente_fad->current()."".$annee,
        'date_recep' => $faker->dateTime(),
        'designation' => $faker->text,
        'observation' => $faker->text,
        'montant' => $faker->randomFloat(),
