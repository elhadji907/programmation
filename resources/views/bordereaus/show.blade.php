        ...
        <div class="container-fluid">
            <div class="card">
                @foreach ($bordereaus as $bordereau)
                    <div class="card-body">
                        <h1 class="h4 card-title text-center h-100 text-uppercase mb-4"><b>{!! $bordereau->courrier->types_courrier->name !!}
                                {!! ' ' !!} N° : </b><span
                                class="font-italic">{{ $bordereau->numero_mandat ?? 'Aucun numéro' }}</span></h1>
                        {{-- <h4 class="card-category"><b><u>Objet</u> : </b>{!! $bordereau->courrier->objet ?? 'Aucun objet' !!}</h4> --}}
                        <p><b><u class="h4">Designation</u> : </b>{{ $bordereau->designation ?? 'Aucune designation' }}</p>
                        <p><b><u class="h4">Observation</u> : </b>{{ $bordereau->observation ?? 'Aucune observation' }}</p>
