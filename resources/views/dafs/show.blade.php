...
@section('title', 'ONFP - Fiche Couriers dafs')
        ...
        <div class="container">
            <div class="container-fluid">
                <div class="card">
                    @foreach ($dafs as $daf)  
                    <div class="card-body">
                        <h3 class="card-title">{!! $daf->courrier->types_courrier->name !!}</h5>
                        <h5 class="card-category">{!! $daf->courrier->objet !!}</h5>
                        <p>{{ $daf->courrier->message }}</p>

                        <div class="d-flex justify-content-between align-items-center">
                            <small>Posté le {!! Carbon\Carbon::parse($daf->courrier->created_at)->format('d/m/Y à H:i:s') !!}</small>
                            <span class="badge badge-primary">{!! $daf->courrier->user->firstname !!}&nbsp;{!! $daf->courrier->user->name !!}</span>
                        </div>

                        <div class="d-flex justify-content-between align-items-center mt-3">
                            @can('update', $daf->courrier)     
                            <a href="{!! url('dafs/' .$daf->id. '/edit') !!}" title="modifier" class="btn btn-outline-warning">
                                <i class="far fa-edit">&nbsp;Modifier</i>
                            </a>
                            @endcan 
                            <a href="{!! url('courriers/' .$daf->courrier->id. '/edit') !!}" title="voir les d&eacute;tails du courrier" class="btn btn-outline-primary">
                                <i class="far fa-eye">&nbsp;D&eacute;tails</i>
                            </a>
                            {{--  <a href="{!! url('courriers/' .$daf->courrier->id. '/edit') !!}" title="supprimer" class="btn btn-outline-danger">
                ...
                <div class="card">
                    <div class="card-body">
                        <h3 class="card-title text-center">Commentaires</h5>
                            @forelse ($daf->courrier->comments as $comment)
                            <div class="card mt-2">
                                <div class="card-body">
                                    {!! $comment->content !!}
                            ...
                            @endforelse
                            <form method="POST" action="{{ route('comments.store', $daf->courrier->id) }}" class="mt-3">
                                @csrf                                                         
                                 <div class="form-group">
