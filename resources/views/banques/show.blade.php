        ...
        <div class="container-fluid">
            <div class="card">
                @foreach ($banques as $banque)
                    <div class="card-body">
                        {{-- <h1 class="h4 card-title text-center h-100 text-uppercase mb-4"><b>{!! $banque->courrier->types_courrier->name !!} --}}
                        <h1 class="h4 card-title text-center h-100 text-uppercase mb-4"><b>{!! ("Frais bancaire") !!}
                                {!! ' ' !!} N° : </b><span
                                class="font-italic">{{ $banque->courrier->numero ?? 'Aucun numéro' }}</span></h1>
                        {{-- <h4 class="card-category"><b><u>Objet</u> : </b>{!! $banque->courrier->objet ?? 'Aucun objet' !!}</h4> --}}
                        <p><b><u class="h4">Designation</u> : </b>{{ $banque->designation ?? 'Aucune designation' }}
                        </p>
                        <p><b><u class="h4">Observation</u> : </b>{{ $banque->observation ?? 'Aucune observation' }}
                        </p>
                        <div class="d-flex justify-content-between align-items-center pt-2">
                            <p><b><u class="h4">Montant </u> : </b><span
                                    class="font-italic">{{ $banque->courrier->total ?? 'Aucune montant' }}</span>{!! ' F CFA' !!}
                            </p>
                            <span><b><u class="h4">Date mandat</u> : </b>{!! Carbon\Carbon::parse($banque->date_mandat)->format('d/m/Y') ?? 'Pas encore mandaté' !!}</span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center pt-2">
                            <small>Posté le {!! Carbon\Carbon::parse($banque->courrier->created_at)->format('d/m/Y à H:i:s') !!}</small>
                            <span class="badge badge-primary">{!! $banque->courrier->user->firstname !!}&nbsp;{!! $banque->courrier->user->name !!}</span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center mt-3">
                            @can('update', $banque->courrier)
                                <a href="{!! url('banques/' . $banque->id . '/edit') !!}" title="modifier" class="btn btn-outline-warning">
                                    <i class="far fa-edit">&nbsp;Modifier</i>
                                </a>
                            @endcan
                            <a href="{!! url('courriers/' . $banque->courrier->id . '/edit') !!}" title="voir les d&eacute;tails du courrier"
                                class="btn btn-outline-primary">
                                <i class="far fa-eye">&nbsp;D&eacute;tails</i>
                            </a>
                            {{-- <a href="{!! url('courriers/' .$banque->courrier->id. '/edit') !!}" title="supprimer" class="btn btn-outline-danger">
           ...
           {{--  <hr>
            <div align="center">
                <a href="{!! url('banques/' . $banque->id) !!}" title="voir les d&eacute;tails du courrier"
            ...
            <div class="card">
                <div class="card-body">
                    <h3 class="card-title text-center">Commentaires</h5>
                        @forelse ($banque->courrier->comments as $comment)
                            <div class="card mt-2">
                                <div class="card-body">
                                    {!! $comment->content !!}
                        ...
                        @endforelse
                        <form method="POST" action="{{ route('comments.store', $banque->courrier->id) }}" class="mt-3">
                            @csrf
                            <div class="form-group">
